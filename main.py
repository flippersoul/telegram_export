...
print(f"{bcolors.WARNING}Скрипт для экспорта Telegram{bcolors.HEADER}")
...
async def main():
    async with app:
        # Для бота await app.send_message('me', 'ping!')

        global chat_ids
        all_dialogs_id = await Chat(app).get_ids()
        chat_ids += all_dialogs_id

        # Вывод id чатов print(chat_ids)
        for cid in chat_ids:
            # Для чатов с telegram api, каналов у которых id  -100 префикс (Для приватных)
            ...
            if chat.type == ChatType.PRIVATE:
                ...
                # Для чатов с telegram api, каналов у которых id  -100 префикс
...
def convert_to_unixtime(date: datetime):
    # Формат даты в telegram: "2022-07-10 08:49:23"
